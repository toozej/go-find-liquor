...
func rootCmdRun(cmd *cobra.Command, args []string) {
	...
	if once {
		if err := r.RunOnce(ctx); err != nil {
			log.Fatalf("Failed to run single search: %v", err)
		}
	} else {
		log.Infof("Starting continuous search with interval %d", conf.Interval)
		if err := r.Start(ctx); err != nil {
			log.Fatalf("Failed to run continuous search: %v", err)
		}
	}
}
